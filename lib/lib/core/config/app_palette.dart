import 'package:flutter/material.dart';

class AppPalette {
  // primary
  static const background = Color(0xFFFFF2D8);
  static const primary = Color(0xFF003F97);
  static const primaryLight = Color(0XFF007EFF);
  static const primaryLighter = Color(0XFF8AD4FF);
  static const whiteColor = Color(0xFFFFFFFF);
  static const blackColor = Color(0XFF000B3A);
  static const startColor = Color(0XFFFFAE17);
  static const disableColor = Color(0XFF373737);
  static const errorColor = Colors.red;
  static const greyColor = Color(0xFF8C8C8C);
  static const lightGrey = Color(0xFFF0EEED);

  // Text
  static const textPrimary = primary;
  static const textPrimaryLight = primaryLight;
  static const textLightBlue = primaryLighter;
  static const textGrey = Color(0xFF8C8C8C);
  static const textWhite = Color(0xFFFFFFFF);
  static const textBlack = blackColor;

  // Gradient text
  static const gradientOne = Color(0XFFA4F1F7);
  static const gradientTwo = Color(0XFF0ECDFA);
  static const gradientThree = Color(0XFF0157B4);
}
